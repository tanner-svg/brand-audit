...
function normalizeReport(vv, sc, companyName, assets) {
  const checklist = [];
  const VALID_SEVERITIES = ["ok", "warning", "issue"];
  for (let i = 0; i < 12; i++) {
    ...
    if (!applicable) {
      checklist.push({ severity: "ok", note: "", not_applicable: true });
    } else {
      const severity = VALID_SEVERITIES.indexOf(item.severity) !== -1 ? item.severity : "ok";
      checklist.push({ severity: severity, note: severity === "ok" ? "" : (item.note || ""), not_applicable: false });
    }
  }
  return {
