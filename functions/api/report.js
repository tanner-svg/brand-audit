...
function normalizeReport(vv, sc, companyName) {
  const checklist = [];
  for (let i = 0; i < 12; i++) {
    const item = (sc.execution_checklist && sc.execution_checklist[i]) || {};
    checklist.push({ flagged: !!item.flagged, note: item.note || "" });
  }
  return {
